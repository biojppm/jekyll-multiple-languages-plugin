...
module Jekyll
  ...
  class Site
    ...
    def process
      ...
      # dictionary to hold missing translations
      self.config['tr_missing'] = []
      ...
      tr_missing = self.config['tr_missing']
      if ! tr_missing.empty?
        y = {tr: tr_missing}
        f = "#{self.source}/_i18n/missing.yml"
        puts "Dumping missing translations to #{f}"
        File.open(f, 'w') { |file| file.write(y.to_yaml) }
      end

    end


    def tr(key, add_missing_key=true)
      lang = self.config['lang']
      fblang = self.config['default_lang'] # fallback lang
      ...
      if translation.nil? or translation.empty?
        # tr does not exist; try the fallback lang
        translation = self.parsed_translations[fblang].access(key) if key.is_a?(String)
        if translation.nil? or translation.empty?
          # fallback lang does not exist. use the key.
          translation = key
          if add_missing_key
            self.miss(key)
          end
          print "i18n: missing key '#{key}'. fallback to key.\n"
        else
          print "i18n: missing #{lang} for key '#{key}' ... "
          print "fallback to '%s' from default language: %s\n" %[translation, self.config['default_lang']]
        end
      end
      translation
    end
    ...
    def tr_args(key, args)
      s = self.tr(key)
      # symbolize keys
      d = {}
      args.keys.each do |k|
        v = args[k]
        vv = self.tr(v, :add_missing => false)
        d[(k.to_sym rescue k) || k] = vv
      end
      tr = s % d
      tr
    end
  ...
  #
  # Localization by getting localized text from YAML files, formatted with args.
  # User must use the "ta" or "translate_args" liquid tags.
  ##############################################################################
  class LocalizeArgsTag < Liquid::Tag
    ...
    def render(context)
      ...
      key = input_split[0].strip
      args = input_split[1] ? input_split[1].strip : "{}"
      jdata = JSON.parse(args)

      key = Liquid::Template.parse(key).render(context)  # Parses and renders some Liquid syntax on arguments (allows expansions)

      site = context.registers[:site] # Jekyll site object
      tr = site.tr_args(key, jdata)
      puts tr
      tr
    end
  end
...
Liquid::Template.register_tag('ta',             Jekyll::LocalizeArgsTag      )
Liquid::Template.register_tag('translate_args', Jekyll::LocalizeArgsTag      )
